...
const combineClientRouter = (dir) => {
  ...
  const getComponent = (file, dirName, comp) => path.join(file.replace(`${dirName}${path.sep}`, '').replace(`${path.sep}moduleConfig.js`, ''), comp).replace(/\\/g, '/');
  ...
  rd.readFilterSync(dir, /(moduleConfig.js)$/)
    .forEach((file) => {
      ...
      containers.forEach((container) => {
        if (file === path.resolve(global.__ROOT_PATH__, `shared${path.sep}page${path.sep}app${path.sep}moduleConfig.js`)) {
          rootRouter.push({
            path: '/',
        ...
        router.push({
          path: (`/${file.replace(`${dir}${path.sep}`, '').replace(`${path.sep}moduleConfig.js`, '')}${container.path || path.sep}`).replace(/\\/g, '/'),
          component: getComponent(file, dir, container.component)
        });
      });
    });
...
const combineClientReducer = (dir, excludePath) => {
  ...
  rd.readFilterSync(dir, /(reducer.js)$/).forEach((file) => {
    ...
    reducers[`${file.replace(`${dir}${path.sep}`, '').replace(`${path.sep}reducer.js`, '').replace(/(\/|\\)/g, '_')}`] = (`${file.replace(`${dir}${path.sep}`, '')}`).replace(/\\/g, '/');
  });
...
const combineSaga = (dir, excludePath) => {
  ...
  rd.readFilterSync(dir, /(saga.js)$/).forEach((file) => {
    if (file.indexOf(excludePath) !== -1) return;
    saga.push((`${file.replace(`${dir}${path.sep}`, '')}`).replace(/\\/g, '/'));
  });
