...
const combineClientRouter = (dir) => {
  ...
  const getComponent = (file, dirName, comp) => path.join(file.replace(`${dirName}${path.sep}`, '').replace(`${path.sep}moduleConfig.js`, ''), comp);
...
const combineClientReducer = (dir, excludePath) => {
  ...
  rd.readFilterSync(dir, /(reducer.js)$/).forEach((file) => {
    ...
    reducers[`${file.replace(`${dir}${path.sep}`, '').replace(`${path.sep}reducer.js`, '').replace(/(\/|\\)/g, '_')}`] = `${file.replace(`${dir}${path.sep}`, '')}`;
  });
...
const combineSaga = (dir, excludePath) => {
  ...
  rd.readFilterSync(dir, /(saga.js)$/).forEach((file) => {
    if (file.indexOf(excludePath) !== -1) return;
    saga.push(`${file.replace(`${dir}${path.sep}`, '')}`);
  });
