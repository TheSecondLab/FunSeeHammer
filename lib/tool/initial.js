...
const initGlobalVariable = () => {
  ...
  // 实际项目中FunSee的文件夹位置
  const funseeFolder = rd.readDirFilterSync(process.cwd(), /(\/funsee|\\funsee)$/);
  ...
  process.env.NODE_ENV = process.env.NODE_ENV || 'dev'; // 如果DEPLOY_ENV为空则默认为开发环境
  global.__NODE_ENV__ = process.env.NODE_ENV;

  console.log(`FunSeeHammer - ${version}: initialize global varible done`);
};
